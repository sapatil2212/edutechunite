...
export default function HomeworkPage() {
  ...
  useEffect(() => {
    const fetchHomework = async () => {
      try {
        const res = await fetch('/api/institution/homework')
  ...
  const getFilteredHomeworks = () => {
    ...
    return homeworks.filter(hw => {
      ...
      const isPending = !hw.submission || hw.submission.status === 'PENDING'
      const isSubmitted = hw.submission && ['SUBMITTED', 'LATE_SUBMITTED', 'EVALUATED'].includes(hw.submission.status)
  ...
  const getStatusBadge = (homework: Homework) => {
    ...
    if (!homework.submission || homework.submission.status === 'PENDING') {
      if (dueDate < now) {
    ...
    if (homework.submission.status === 'SUBMITTED' || homework.submission.status === 'LATE_SUBMITTED') {
      return <Badge variant="primary">Submitted</Badge>
    }
    
    if (homework.submission.status === 'EVALUATED') {
  ...
  const pendingCount = homeworks.filter(hw => (!hw.submission || hw.submission.status === 'PENDING') && new Date(hw.dueDate) >= new Date()).length
  const overdueCount = homeworks.filter(hw => (!hw.submission || hw.submission.status === 'PENDING') && new Date(hw.dueDate) < new Date()).length
  const submittedCount = homeworks.filter(hw => hw.submission && ['SUBMITTED', 'LATE_SUBMITTED', 'EVALUATED'].includes(hw.submission.status)).length
  const dueTodayCount = homeworks.filter(hw => {
    const today = new Date().toISOString().split('T')[0]
    const due = new Date(hw.dueDate).toISOString().split('T')[0]
    return due === today && (!hw.submission || hw.submission.status === 'PENDING')
  }).length
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <Card className="border-none shadow-soft p-12 text-center">
          ...
          <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-2">No homework found</h3>
          <p className="text-gray-500 dark:text-gray-400">
            {filter === 'all' ? 'No homework has been assigned yet.' : `No ${filter} homework at the moment.`}
          </p>
        </Card>
      ) : (
          ...
          {filteredHomeworks.map((homework) => (
            ...
            >
              <div className="p-6">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="flex items-start gap-4">
                    <div
                      ...
                      style={{ backgroundColor: homework.subject?.color || '#6B7280' }}
                    >
                      <span className="text-xs font-bold opacity-80 uppercase leading-none mb-1">
                        {homework.subject?.code?.substring(0, 3) || 'HW'}
                      </span>
                  ...
                  <div className="flex items-center gap-4 ml-18 md:ml-0">
                    <div className="text-right hidden sm:block">
                      <div className="flex items-center gap-1.5 text-sm font-bold text-gray-900 dark:text-white">
                        ...
                        {new Date(homework.dueDate).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}
                      </div>
                      <p className={`text-xs font-bold mt-0.5 ${
                        new Date(homework.dueDate) < new Date() && (!homework.submission || homework.submission.status === 'PENDING')
                          ? 'text-red-500'
                          : 'text-gray-500'
                      }`}>
                        {getDaysLeft(homework.dueDate)}
                      </p>
                    </div>
                    <Link
                ...
                {homework.submission?.status === 'EVALUATED' && (
                  <div className="mt-4 flex items-center justify-between p-3 bg-green-50 dark:bg-green-900/20 rounded-xl border border-green-100 dark:border-green-900/30">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-bold text-green-800 dark:text-green-300">
                        Score: {homework.submission.marksObtained}/{homework.maxMarks}
                      </span>
                    </div>
