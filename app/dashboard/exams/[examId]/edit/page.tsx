...
export default function EditExamPage() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [examRes, yearsRes, unitsRes] = await Promise.all([
        fetch(`/api/exams/${examId}`),
        fetch("/api/institution/academic-years"),
        fetch("/api/institution/academic-units"),
      ]);

      const [examData, yearsData, unitsData] = await Promise.all([
        examRes.json(),
        yearsRes.json(),
        unitsRes.json(),
      ]);
      ...
      if (examData.success) {
        ...
        setFormData({
          ...
          overallPassingPercentage: exam.overallPassingPercentage,
          subjectWisePassing: exam.subjectWisePassing,
          showRank: exam.showRank,
          showPercentage: exam.showPercentage,
          showGrade: exam.showGrade,
          allowMarksCorrection: exam.allowMarksCorrection || false,
        ...
        <div className="p-6 lg:p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex items-center justify-end gap-4">
              ...
              <button
                type="submit"
                disabled={saving || formData.targetClasses.length === 0}
